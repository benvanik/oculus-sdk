...
-(void) keyDown:(NSEvent*)ev
{
    ...
    if ([chars length])
    {
        ...
        unsigned mods = MapModifiers([ev modifierFlags]);
...
bool PlatformCore::SetupWindow(int w, int h)
{
    ...
    NSWindow* win = [[NSWindow alloc] initWithContentRect:winrect styleMask:NSTitledWindowMask backing:NSBackingStoreBuffered defer:NO];
